      ...
      <div className={`${editing ? "hover:border-muted-foreground transition-all duration-200" : "group relative"}`}>
        {editing && (
          <div className='absolute top-2 right-2 md:hidden z-10'>
            <Button variant='ghost' size='sm' className='h-8 w-8 p-0' onClick={() => setShowControls(!showControls)}>
        ...
        {(editing || showControls) && (
          <div className='flex justify-between items-center mb-2'>
            <div
              className={`flex items-center gap-2 ${editing ? "cursor-default" : "cursor-move"}`}
              {...attributes}
              {...listeners} // ✅ Always attach listeners so drag is clickable
            >
